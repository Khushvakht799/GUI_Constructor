...
Запускается из main.py (см. инструкцию ниже).
...
class GUIManager:
    ...
    def _build_ui(self):
        ...
        btn_refactor = ttk.Button(toolbar, text='🔧 Рефакторинг', command=self.on_refactor)
        ...
        btn_plugins = ttk.Button(toolbar, text='🧩 Плагины', command=self.on_plugins)
        btn_plugins.pack(side=tk.LEFT, padx=4)
        ...
        # Начальная загрузка дерева проекта
    ...
    def _run_analyzer_subprocess(self, project_path: str):
        # Запускаем python core/project_analyzer.py --out analyzer_report.json
        ...
        try:
            ...
            if proc.returncode == 0:
                ...
                self.log(f'Анализ завершился с кодом {proc.returncode}')
    ...
    def load_tree_from_report(self):
        ...
        self.log('Дерево загружено из отчета')
    ...
    def on_open_kb(self):
        # Открываем knowledge.json в редакторе
        if not KB_FILE.exists():
            # создаем минимальный каркас
    ...
    def on_refactor(self):
        # Заглушка — вызов рефакторинг-модуля
    ...
    def on_plugins(self):
        """Открыть окно с плагинами"""
        plugins_dir = ROOT / "plugins"
        if not plugins_dir.exists():
            self.log("Папка plugins не найдена")
            return
        ...
        canvas = tk.Canvas(wnd)
        scrollbar = ttk.Scrollbar(wnd, orient="vertical", command=canvas.yview)
        ...
        for py_file in plugins_dir.glob("*.py"):
            ...
            try:
                spec = __import__(f"plugins.{plugin_name}", fromlist=["Plugin"])
                plugin_cls = getattr(spec, "Plugin", None)
                if plugin_cls and callable(getattr(plugin_cls, "run", None)) and callable(getattr(plugin_cls, "name", None)):
                    btn = ttk.Button(
                        scroll_frame,
                        text=plugin_cls().name(),
                        command=lambda cls=plugin_cls: cls().run(self)
                    )
                    btn.pack(fill="x", pady=2, padx=2)
                else:
                    self.log(f"Плагин {plugin_name} не содержит класс Plugin с методами name() и run()")
            except Exception as e:
                self.log(f"Ошибка загрузки плагина {plugin_name}: {e}")
